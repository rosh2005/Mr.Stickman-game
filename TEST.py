...
def within_y(co1, co2):
    if (co1.y1 > co2.x1 and co1.y1 < co2.y2) \
...
class StickFigureSprite(Sprite):
    ...
    def move(self):
        ...
        bottom = True
        falling = True
        # Check that the figure does not fall through the bottom
        if self.y > 0 and co.y2 >=self.game.canvas_height:
            self.y = 0
            bottom = False
        # Check if the figure is hitting the top
...
def within_y(co1, co2):
    ...
    or (co1.y2 > co2.y1 and co1.y2 < co2.y2) \
    or (co2.y1 > co1.y1 and co2.y2 < co1.y1) \
    or (co2.y2 > co1.y1 and co2.y2 < co1.y1):
        return True
    else:
        return False
